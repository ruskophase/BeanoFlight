...
evidence by default, with a target and hard maximum of two inference-attached
observations per bean. After inference drains, a confirmed bean with no stereo
sample receives a clearly marked CamL or geometry-only fallback. The
lower-priority collector creates no bean images; see
[Live Statistics Capture](live-statistics-capture.md).
