...
class StatisticsDashboardTests(unittest.TestCase):
    def test_dashboard_is_self_contained_and_json_safe(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            stylesheet = (output / "dashboard.css").read_text(encoding="utf-8")
            ...
            self.assertIn('id="summary-rate-metrics"', html)
            self.assertIn('id="health-rate-metrics"', html)
            self.assertIn('metric("Confirmed beans / second"', script)
            self.assertIn('metric("Inferences / second"', script)
            self.assertIn('metric("Measurements / second"', script)
            self.assertIn("height: 11px;", stylesheet)
